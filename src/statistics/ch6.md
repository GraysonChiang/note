...
|超幾何||${ c_x^s c_{n-x}^{N-s} \over  c_n^N }$|$n \times p$|${ N - n \over N-1} np(1-p)$|
|幾何|$X \sim Geo(p)$|$q^{x-1} p,x = 1,2,...$|${ \mu } = {q \over p } ={ 1 - p \over p }$|${ \sigma^2 } = { q \over p^2} = { 1 - p \over p^2 }$|
|Poisson|$X \sim Poisson(\lambda t)$|${ e^{-\lambda t}  \times {\lambda t}^x \over x !} , x = 0,1,2,3, ...$|$\lambda$|$\lambda$|
